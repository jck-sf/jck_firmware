...
bool DIGITAL_RAIN(effect_params_t* params) {
    ...
    for (uint8_t row = 0; row < MATRIX_ROWS; row++) {
        for (uint8_t col = 0; col < MATRIX_COLS; col++) {
            ...
            uint8_t led[LED_HITS_TO_REMEMBER];
            if (rgb_matrix_map_row_column_to_led(row, col, led) == 0) continue;
            ...
            if (timer_expired32(g_rgb_timer, digital_rain_timer)) g_rgb_frame_buffer[led[0]] = qsub8(g_rgb_frame_buffer[led[0]], 1);

            uint8_t led_prev[LED_HITS_TO_REMEMBER];
            if (row == 0 || rgb_matrix_map_row_column_to_led(row - 1, col, led_prev) == 0) continue;
